...
def finetuning(image):
    ...
    if Finetune:
        if os.path.isfile(FinetuningFile):
            ...
            cv2.createTrackbar('rho', 'Hough', 1, 10, lambda _: None)
            cv2.createTrackbar('theta', 'Hough', 1, 360, lambda _: None)
            cv2.createTrackbar('threshold', 'Hough', 1, 500, lambda _: None)
            cv2.createTrackbar('minLineLength', 'Hough', 1, 100, lambda _: None)
            cv2.createTrackbar('maxLineGap', 'Hough', 1, 100, lambda _: None)

            while True:
                rho = cv2.getTrackbarPos('rho', 'Hough')
                theta = cv2.getTrackbarPos('theta', 'Hough') * np.pi / 180
